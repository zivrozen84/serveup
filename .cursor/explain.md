...
| קובץ | מטרה |
|------|------|
| `page.tsx` | תצוגת תפריט מסעדה לפי slug – טעינה מ־Prisma, העברת restaurant + categories ל־RestaurantMenu עם phoneLayout (מסגרת טלפון). |
| `order/page.tsx` | כניסה לטרמינל – POST ל־session, הפניה ל־`/r/[slug]/order/[token]`. |
| `order/[token]/page.tsx` | דף טרמינל – תפריט במסגרת טלפון (אותם ממדים כמו Preview), OrderSessionProvider, RestaurantMenu עם phoneLayout. |
| `order/[token]/summary/page.tsx` | סוכם הזמנה – **תמיד במסגרת טלפון** (אותם ממדים כ־Preview והתפריט), לא מסך מלא; רקע frameUrl אם קיים, header + OrderSummaryPage. |
...
## מסך והדמיית טלפון – ממדים ואפליקציה

**כל זרימת האפליקציה (תפריט, טרמינל, סוכם הזמנה)** מוצגת **תמיד במסגרת טלפון** – אותם ממדים כמו ב־Preview בעריכת מסעדה (רוחב 420px, פינות מעוגלות, "שקע" עליון). לא מסך מלא: הדפים `/r/[slug]`, `/r/[slug]/order/[token]` ו־`/r/[slug]/order/[token]/summary` עטופים באותה מסגרת כדי שהחוויה תהיה עקבית (אפליקציה).

**תצוגת הטלפון קבועה מראש:** אי אפשר להקטין אותה, להזיז אותה או לשנות את גודלה – **אפשר רק לבנות עליה**. גובה המסגרת קבוע (למשל `min(90vh, 700px)`), כך שאם יש מעט תוכן (למשל פריטים בודדים בסוכם) – המסגרת **לא** מתכווצת; האזור הריק מתמלא ב**רקע מת** (צבע או תמונת רקע), והטלפון נשאר בגודל אחיד. חובה לשמור על אותו גודל טלפון בכל המסכים (תפריט, סוכם הזמנה, Preview).
...
- **Prisma – לא למחוק נתונים:** לא לרצות או להציע איפוס DB / מחיקת נתונים. תמיד להציע `npx prisma generate` או `npx prisma db push` (לעדכון סכמה בלי למחוק). אם Prisma שואל "We need to reset... All data will be lost" – להמליץ להשיב **N** (לא לאפס).
...
- **סוכם הזמנה במסגרת טלפון:** דף סוכם ההזמנה (`/r/[slug]/order/[token]/summary`) מוצג תמיד באותה מסגרת טלפון (420px, notch, רקע frameUrl) כמו התפריט ו־Preview – לא במסך מלא.

---

## להזכיר – לעשות מאוחר יותר

(המשתמש ביקש להזכיר לו לבצע את הדברים הבאים בהמשך.)

1. **דף פתיחה / הסבר קל ללקוח** – דף הסבר קצר לפני התפריט (למשל שהעגלה משותפת לשולחן, איך זה עובד). נדחה: "אנחנו נעשה להם סוג של דף פתיחה הסבר קל, אז נחכה עם זה".
2. **סיכום לפני שליחה למטבח** – לפני שליחת ההזמנה: להציג סיכום ברור (רשימה + סה"כ) וכפתור "שולח להכנה" / "שליחה למטבח". נדחה: "עדיין לא, נכונה את זה".
3. **מסך המסעדה (השלישי)** – מסך ייעודי למסעדה (לא מפתחים, לא לקוחות) – שם יוצגו ההזמנות, סטטוסים וכו'. נדחה: "מסך שלישי שלא נבנה עדיין – המסעדה (שם יהיה כתוב ההזמנות, וכו)".
   - **הנגשה למסעדנים – להזכיר:** איך מנגישים את המסך השלישי למסעדנים? אפשרויות: **טאבלט**, **מחשב טאצ'**, או האופציה שהציע ג'מיני – **מסך טאבלט גדול עם מדפסת** (מעין תחנת קבלת הזמנות).
